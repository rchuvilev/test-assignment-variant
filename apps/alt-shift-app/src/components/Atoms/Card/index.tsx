...
import {CONST_TEXT_CARD_ACTION_COPY, CONST_TEXT_CARD_ACTION_DELETE} from "../../../misc/consts.ts";
...
const CardAction = ({actionType, cardText}: TCardActionProps) => {
    ...
    return (
        <div onClick={handleClick} className={styles.CardAction}>
            <img className={styles.CardActionImage} src={iconUrl} alt={`${name} icon`}/>
            <TypographyText
...
const Component = ({cardText, maxHeightPx}: TProps) => {
    ...
    return cardText && (
        <div
            ...
            style={isCropped ? {[`--max-height`]: `${maxHeightPx}px`} as CSSProperties : {}}
        >
            <div className={clsx(styles.Content, {[styles.__cropped]: isCropped})}>
                {cardText.split('\n').map((line: string) => (
                    <p><TypographyText size={18} color="var(--font-color-gray)">{line}</TypographyText></p>
                ))}
            </div>
            {isCropped && (
                <div className={styles.Footer}>
                    {
                        (['copy', 'delete'] as TCardActionType[])
                            .map((actionType: TCardActionType) => <CardAction cardText={cardText}
